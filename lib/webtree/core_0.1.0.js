...
/**
 The `Core` module provides basic functionality present in most applications.
...
/*global wtCore*/
wtCore.service('sessionStorageProxy', ['$rootScope', '$http', 'wtCoreConfig', function ($rootScope, $http, config) {
    'use strict';
    ...
    this.authenticate = function (login, password) {
        ...
        $http.post(config.authentication.url, {login: login, password: password}).success(onSuccess).failure(onError);
    };
